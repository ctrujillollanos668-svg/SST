...
{
    /**
     * Muestra el catálogo de Actos Inseguros para el Aprendiz SST
     */
    public function index()
    {
        if (!Auth::check()) {
            return redirect()->route('login', ['redirect' => route('SST.aprendiz.tipos_eventos.actos_inseguros.index')]);
        }
        ...
        if ($user->roles->contains('slug', 'sst.aprendiz') || $user->roles->contains('slug', 'sst.funcionario') || $user->roles->contains('slug', 'sst.admin') || $user->hasSuperAdmin()) {
            $actosInseguros = TipoEvento::where('tipo_categoria', 'acto_inseguro')
                ->where('estado', 'activo')
                ->orderBy('nombre', 'asc')
                ->get();

            return view('sst::aprendiz.tipos_eventos.Acto_inseguros', compact('actosInseguros'));
        }
    ...
    /**
     * Registra un nuevo acto inseguro desde el panel de Aprendiz
     */
    public function store(Request $request)
    {
        ...
        return redirect()->route('SST.aprendiz.tipos_eventos.actos_inseguros.index')
