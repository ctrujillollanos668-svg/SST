...
{
    /**
     * Muestra el catálogo de Tipos de Emergencias para el Aprendiz SST
     */
    public function index()
    {
        if (!Auth::check()) {
            return redirect()->route('login', ['redirect' => route('SST.aprendiz.tipos_eventos.tipos_emergencias.index')]);
        }
        ...
        if ($user->roles->contains('slug', 'sst.aprendiz') || $user->roles->contains('slug', 'sst.funcionario') || $user->roles->contains('slug', 'sst.admin') || $user->hasSuperAdmin()) {
            $tiposEmergencias = TipoEvento::where('tipo_categoria', 'tipo_emergencia')
                ->where('estado', 'activo')
                ->orderBy('nombre', 'asc')
                ->get();

            return view('sst::aprendiz.tipos_eventos.Tipos_emergencias', compact('tiposEmergencias'));
        }
    ...
    /**
     * Registra un nuevo tipo de emergencia desde el panel de Aprendiz
     */
    public function store(Request $request)
    {
        ...
        return redirect()->route('SST.aprendiz.tipos_eventos.tipos_emergencias.index')
