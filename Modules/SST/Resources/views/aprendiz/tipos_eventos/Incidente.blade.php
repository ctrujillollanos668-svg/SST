...
@section('title', 'Incidentes de Trabajo • SST')
        ...
        <div>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-100">
                <div class="flex items-center gap-3.5">
                    <div class="w-12 h-12 rounded-2xl bg-amber-50 text-amber-500 flex items-center justify-center text-xl border border-amber-100/80 shadow-2xs shrink-0">
                        <i class="fa-solid fa-triangle-exclamation"></i>
                    </div>
                    <div>
                        <h1 class="text-xl sm:text-2xl font-extrabold text-slate-800 tracking-tight">Incidentes de Trabajo</h1>
                ...
                @if($incidentes->count() > 0)
                    ...
                    <div class="mb-5 flex flex-col sm:flex-row items-center justify-between gap-3">
                        <div class="w-full sm:w-80 relative">
                            ...
                            <input type="text" id="inputBuscarEvento" placeholder="Buscar incidente..." 
                                class="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition">
                        </div>
                        <span class="text-xs font-bold text-slate-400 self-end sm:self-auto">
                            <span id="conteoEventos" class="text-slate-700">{{ $incidentes->count() }}</span> {{ $incidentes->count() == 1 ? 'incidente registrado' : 'incidentes registrados' }}
                        </span>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4" id="contenedorEventos">
                        @foreach($incidentes as $item)
                            <div class="item-card bg-slate-50/50 hover:bg-white p-4 sm:p-5 rounded-2xl border border-slate-200/80 shadow-2xs hover:shadow-md hover:border-amber-300 transition-all space-y-2 cursor-pointer relative group"
                                ...
                                <div class="flex items-start justify-between gap-3">
                                    <h3 class="font-bold text-slate-800 text-sm flex items-center gap-2">
                                        ...
                                        <span class="group-hover:text-amber-600 transition-colors">{{ $item->nombre }}</span>
                                    </h3>
                    ...
                    <div id="noResultsMsg" class="hidden text-center py-12">
                        <p class="text-xs font-bold text-slate-400">No se encontraron incidentes con ese criterio de búsqueda.</p>
                    </div>
                @else
    ...
    <div class="bg-white rounded-3xl shadow-2xl border border-slate-100 max-w-lg w-full p-6 transform transition-transform duration-200 scale-95" onclick="event.stopPropagation()">
        ...
        <div class="flex items-center justify-between pb-4 border-b border-slate-100">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="font-extrabold text-sm text-slate-800 tracking-tight">Registrar Incidente de Trabajo</h3>
                    <p class="text-[11px] text-slate-400">Ingresa la información del nuevo tipo de incidente</p>
                </div>
            </div>
        ...
        <form action="{{ route('SST.aprendiz.tipos_eventos.incidentes.store') }}" method="POST" class="pt-4 space-y-4">
            @csrf

            <div class="space-y-1">
                <label class="block text-xs font-bold text-slate-700">Nombre del Incidente <span class="text-rose-500">*</span></label>
                <input type="text" name="nombre" required placeholder="Ej: Casi caída de escaleras, Derrame menor..." 
                    class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition">
            </div>

            <div class="space-y-1">
                <label class="block text-xs font-bold text-slate-700">Área o Módulo al que Pertenece</label>
                <input type="text" name="modulo_pertenece" placeholder="Ej: Pasillos, Biblioteca, General..." 
                    class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition">
            </div>

            <div class="space-y-1">
                <label class="block text-xs font-bold text-slate-700">Descripción Técnica</label>
                <textarea name="descripcion" rows="3" placeholder="Describe brevemente las características de este incidente..." 
                    class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition resize-none"></textarea>
            </div>
    ...
    <div class="bg-white rounded-3xl shadow-2xl border border-slate-100 max-w-lg w-full p-6 transform transition-transform duration-200 scale-95" onclick="event.stopPropagation()">
        ...
        <div class="flex items-center justify-between pb-3.5 border-b border-slate-100">
            <div class="flex items-center gap-2.5 text-slate-800">
                ...
                <h3 class="font-bold text-sm tracking-tight">Detalle del Incidente</h3>
            </div>
        ...
        <div class="pt-4 space-y-3">
            <div class="flex items-center justify-between gap-2">
                ...
                <span id="modalDetalleModulo" class="inline-flex items-center px-2.5 py-0.5 rounded-lg text-[10px] font-bold bg-amber-50 text-amber-700 border border-amber-200 shrink-0"></span>
            </div>
