                        ...
                        <tr class="hover:bg-slate-50/60 transition-colors">
                            ...
                            <td class="py-4 px-6 text-slate-600 max-w-md">
                                @if($incidente->descripcion)
                                    <p class="line-clamp-2 cursor-pointer hover:text-orange-600 transition-colors" onclick="showFullDesc(event, {{ json_encode($incidente->descripcion) }})" title="Haz clic para ver la descripción completa">
                            ...
                            <td class="py-4 px-6 text-center whitespace-nowrap">
                                <div class="flex items-center justify-center gap-2">
                                    <button onclick="openEditModal({{ $incidente->id_tipo_evento }}, {{ json_encode($incidente->nombre) }}, {{ json_encode($incidente->descripcion ?? '') }}, {{ json_encode($incidente->estado) }})" class="w-9 h-9 rounded-xl bg-amber-50 hover:bg-amber-100 text-amber-600 border border-amber-200/80 flex items-center justify-center transition-all duration-150 cursor-pointer shadow-xs hover:scale-105" title="Editar">
                                        <i class="fa-regular fa-pen-to-square text-sm"></i>
                                    </button>
                                    <button onclick="openDeleteModal({{ $incidente->id_tipo_evento }}, {{ json_encode($incidente->nombre) }})" class="w-9 h-9 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200/80 flex items-center justify-center transition-all duration-150 cursor-pointer shadow-xs hover:scale-105" title="Eliminar">
                        ...
                        <tr>
                            <td colspan="4" class="py-12 px-6 text-center">
                                <div class="flex flex-col items-center justify-center text-slate-400 space-y-2">
                                    ...
                                    <p class="text-sm font-semibold text-slate-600">No hay incidentes registrados aún.</p>
                                    <p class="text-xs text-slate-400">Haz clic en "Registrar Tipo de Incidentes" para agregar el primero a la base de datos.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
<div id="modalRegister" class="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center hidden p-4">
    <div class="bg-white rounded-2xl shadow-xl border border-slate-100 w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <form action="{{ route('SST.admin.tipos_eventos.incidentes.store') }}" method="POST">
            @csrf
            <div class="p-6 space-y-4 text-xs">
                <div>
                    <label class="block font-bold text-slate-700 mb-1.5">Nombre del Incidente *</label>
                    <input type="text" name="nombre" required placeholder="Ej. Casi caída por piso húmedo" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-100 transition text-sm">
                </div>
                <div>
                    <label class="block font-bold text-slate-700 mb-1.5">Descripción</label>
                    <textarea name="descripcion" rows="3" placeholder="Describa el incidente ocurrido..." class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-100 transition text-sm"></textarea>
                </div>
                <div>
...
<div id="modalEdit" class="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center hidden p-4">
    <div class="bg-white rounded-2xl shadow-xl border border-slate-100 w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <form id="editIncidenteForm" action="" method="POST">
            @csrf
            @method('PUT')
            <div class="p-6 space-y-4 text-xs">
                <div>
...
<div id="modalDelete" class="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center hidden p-4">
    <div class="bg-white rounded-2xl shadow-xl border border-slate-100 w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
        <div class="p-6 text-center">
            ...
            <form id="deleteIncidenteForm" action="" method="POST" class="mt-6 flex items-center justify-center gap-3">
                @csrf
                @method('DELETE')
...
<script>
    ...
    function openEditModal(id, nombre, descripcion, estado) {
        const form = document.getElementById('editIncidenteForm');
        form.action = "{{ url('Sst/admin/tipos-eventos/incidentes') }}/" + id;
    ...
    function openDeleteModal(id, nombre) {
        const form = document.getElementById('deleteIncidenteForm');
        form.action = "{{ url('Sst/admin/tipos-eventos/incidentes') }}/" + id;
